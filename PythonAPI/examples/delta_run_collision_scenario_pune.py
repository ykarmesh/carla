...
class World(object):
    def __init__(self, carla_world, hud, spawn_point):
        ...
        self.restart(spawn_point)
        self.world.on_tick(hud.on_world_tick)
        self._surface = None

    def restart(self, spawn_point_collision):
        # Get a random vehicle blueprint.
        blueprint = random.choice(self.world.get_blueprint_library().filter('vehicle.carlamotors.*'))
...
def game_loop(args):
    ...
    try:
        ...
        npc_spawn_points = [x for x in spawn_points if x is not spawn_points[1]]
        NPCs = collision_scenario.NPC(client.get_world(), 5, npc_spawn_points)
        hud = HUD(args.width, args.height)
        world = World(client.get_world(), hud, spawn_points[2])
        controller = KeyboardControl(world, args.autopilot)

        colliding_agent = collision_scenario.Colliding_Agent(world.world, world.vehicle)
        colliding_agent.create_agent(spawn_points[1])
